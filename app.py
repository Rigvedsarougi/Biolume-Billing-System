...
def generate_invoice(customer_name, selected_products, quantities):
    ...
    # Table Header (adjusted column widths and reduced font size)
    pdf.set_fill_color(200, 220, 255)
    pdf.set_font("Arial", 'B', 9)  # Reduced font size for table headers
    pdf.cell(20, 8, "ID", border=1, align='C', fill=True)
    pdf.cell(65, 8, "Product", border=1, align='C', fill=True)  # Widened for long product names
    pdf.cell(35, 8, "Category", border=1, align='C', fill=True)
    ...
    # Product details with reduced font size
    pdf.set_font("Arial", '', 9)  # Reduced font size for table content
    total_price = 0
    for idx, product in enumerate(selected_products):
        ...
        pdf.cell(20, 8, product_data['Product ID'], border=1)
        pdf.cell(65, 8, product_data['Product Name'], border=1)  # More space for product names
        pdf.cell(35, 8, product_data['Product Category'], border=1)
        pdf.cell(15, 8, str(quantity), border=1, align='C')
        pdf.cell(25, 8, f"{unit_price:.2f}", border=1, align='R')
        pdf.cell(25, 8, f"{item_total_price:.2f}", border=1, align='R')
        total_price += item_total_price
        pdf.ln()

    # Summary Section with reduced font size
    pdf.ln(5)
